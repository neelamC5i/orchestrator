...
export default function ExtractionTab({ data }: { data: ObservatoryData }) {
  ...
  const hasParser = parser.length > 0;
  const hasLineage = lineage.length > 0;

  if (!hasParser && !hasLineage) return <EmptyState message="No extraction data available." />;

  return (
    <div className="space-y-3">
      {(hasParser || hasLineage) && (
        <div className="flex flex-col gap-3 md:flex-row">
          {hasParser && (
            <div className="card md:basis-[35%] md:max-w-[35%]">
          ...
          {hasLineage && (
            <div className="card overflow-auto md:basis-[65%] md:max-w-[65%]">
